...
#define WAIT_FOR_EXPOSURE_BUFFER 1.7
...
asynStatus PSL::openCamera(int cameraId)
{
    ...
    // we have to open the desired camera in PSViewer before starting the server
...
asynStatus PSL::getVersion()
{
    asynStatus status;
    asynUser *pasynUser = pasynUserServer_;
    const char *expectedResponse = "PSViewer-";
...
asynStatus PSL::getConfig()
{
    ...
    if (validOptions_.count("Exposure"))
    {
        ...
        if (status == asynSuccess)
        {
            ...
            waitForExposure_ = exposure + WAIT_FOR_EXPOSURE_BUFFER;
...
asynStatus PSL::getImage()
{
    ...
    else if (strncmp(fromServer_, "I16;", 4) == 0)
    {
        prefixLen = 4;
        dataType = NDUInt16;
    }
